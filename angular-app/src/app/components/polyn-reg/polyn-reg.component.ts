...
@Component({
  ...
  templateUrl: './polyn-reg.component.html',
  styleUrls: ['./polyn-reg.component.css'],
  animations: [
    trigger(
      'enterAnimationLeft', [
        transition(':enter', [
...
export class PolynRegComponent implements OnInit {
  uploader: FileUploader = new FileUploader(
    {
      url: 'http://127.0.0.1:8000/poly_info/', removeAfterUpload: false, autoUpload: true
    }
  );
  visible: boolean;
  visible1: boolean;
  ...
  getSomeInfo() {
    this.api.getPolynomial().subscribe(
      (data: any[]) => {
        this.info = data;
