...
@Component({
  selector: 'app-run-mov-avg',
  templateUrl: './run-mov-avg.component.html',
  styleUrls: ['./run-mov-avg.component.css'],
  animations: [
    trigger(
      'enterAnimationLeft', [
        transition(':enter', [
...
export class RunMovAvgComponent implements OnInit {

  uploader: FileUploader = new FileUploader(
    {
      url: 'http://127.0.0.1:8000/running_ma_info/', removeAfterUpload: false, autoUpload: true
       }
    );
  visible = false;
  visible1 = false;
  ...
  getSomeInfo() {
    this.api.getRunningMovingAverages().subscribe(
     (data: any[]) => {
           this.info = data;
